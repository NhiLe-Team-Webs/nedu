  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <section className="bg-white py-16 md:py-24">
        <div className="container mx-auto px-4 md:px-8">
          <div className="mb-12 max-w-3xl">
            <h2 className="text-4xl md:text-6xl font-semibold tracking-tighter text-gray-900 mb-3 leading-tight">
          ...
          <div className="hidden lg:flex gap-3 h-[580px] mb-20">
            {daysData.map((day, index) => {
              ...
              return (
                <div
                  ...
                  className={`relative rounded-3xl cursor-pointer transition-all duration-700 ease-out-expo overflow-hidden flex flex-col group border border-transparent
                    ${isActive
                      ? "flex-[3] bg-gray-900 shadow-2xl scale-[1.01]"
                ...
                >
                  {isActive && (
                    <div className="h-full flex flex-col relative z-10 p-8">
                      <div className="flex justify-between items-start mb-4 animate-slide-up" style={{ animationDelay: "0ms" }}>
                        <div>
                      ...
                      <div className="flex-1 min-h-0 flex flex-col">
                        <p
                          className={`text-2xl font-medium mb-3 leading-tight animate-slide-up ${
                            isSpecial
                        ...
                        <p className="text-gray-300 text-sm leading-relaxed mb-6 border-l-2 border-primary/50 pl-4 max-w-lg animate-slide-up" style={{ animationDelay: "200ms" }}>
                          {day.desc}
                        </p>
                        ...
                        <div className="mb-6 grid grid-cols-1 gap-2">
                          {day.bullets.map((bullet, idx) => (
                            <div
                              key={idx}
                              className="flex items-center gap-2 group/item animate-slide-up"
                              style={{ animationDelay: `${300 + idx * 100}ms` }}
                            >
                              <ArrowRight size={14} className="text-primary group-hover/item:translate-x-1 transition-transform" />
                        ...
                        <div
                          className="flex flex-wrap gap-2 mb-auto animate-slide-up"
                          style={{ animationDelay: `${300 + day.bullets.length * 100}ms` }}
                        >
                          {day.activities.map((act, i) => (
                            <span
                              key={i}
                              className="px-3 py-1 rounded-full bg-white/10 text-gray-300 text-[10px] font-bold uppercase tracking-wider border border-white/5"
                            >
                              {act}
                            </span>
                          ))}
                      ...
                      <div
                        className="bg-gradient-to-r from-white/10 to-white/5 backdrop-blur-md rounded-xl p-4 border border-white/10 flex items-center gap-4 mt-4 group/box animate-slide-up"
                        style={{ animationDelay: `${400 + day.bullets.length * 100}ms` }}
                      >
                        <div className="p-2 bg-primary text-gray-900 rounded-lg shrink-0 shadow-lg group-hover/box:scale-110 transition-transform duration-300">
                          <Sparkles size={18} />
                        </div>
                        <div>
                          <p className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">Giá trị nhận được</p>
                          <p className="text-white font-medium text-base">{day.outcome}</p>
                        </div>
                      </div>

                      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-b from-brand-yellow/20 to-transparent rounded-full blur-[80px] -mr-32 -mt-32 pointer-events-none animate-pulse-glow"></div>
                    </div>
                  )}

                  {!isActive && (
                    <div className="h-full flex flex-col items-center justify-between py-8 w-full transition-opacity duration-300 delay-100">
                      <span className="text-gray-400 font-bold text-lg">0{day.id}</span>
                      ...
                      <div className="w-10 h-10 rounded-full bg-white border border-gray-100 flex items-center justify-center group-hover:scale-110 group-hover:border-primary transition-all duration-300 shadow-sm">
              ...
              return (
                <div
                  key={day.id}
                    ...
                    isActive ? "bg-gray-900 border-gray-900 shadow-xl scale-[1.02]" : "bg-white border-gray-100 hover:border-gray-200"
                  }`}
                >
                  <div
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <span className={`text-sm font-bold opacity-50 ${isActive ? "text-white" : "text-gray-900"}`}>0{day.id}</span>
                      <div>
                        <h3 className={`text-xl font-bold uppercase tracking-tight leading-none ${isActive ? "text-white" : "text-gray-900"}`}>
                          {day.title}
                        </h3>
                        <p className={`text-xs font-medium uppercase tracking-wider mt-1 ${isActive ? "text-primary" : "text-gray-400"}`}>
                          {day.sub}
                        </p>
                      </div>
                    </div>
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
                        isActive ? "bg-white/10 text-white rotate-180" : "bg-gray-50 text-gray-400"
                      }`}
                    >
                  ...
                  <div className={`grid transition-[grid-template-rows] duration-500 ease-out-expo ${isActive ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                    <div className="overflow-hidden">
                      <div className={`px-5 pb-6 pt-0 transition-opacity duration-500 ${isActive ? "opacity-100 delay-200" : "opacity-0"}`}>
                        ...
                        <p className="text-gray-300 text-sm mb-5 leading-relaxed opacity-90">
                          {day.desc}
                        </p>

                        <div className="mb-5 space-y-2 bg-black/20 p-4 rounded-xl">
                          {day.bullets.map((bullet, idx) => (
      ...
      <section className="bg-white pb-20 md:pb-28">
        <div className="container mx-auto px-4 md:px-8 border-t border-gray-100 pt-16 md:pt-24">
          <div className="mb-12 md:mb-16 max-w-5xl mx-auto px-2 text-center md:text-left">
            <h2 className="text-4xl md:text-7xl font-semibold tracking-tighter text-gray-900 mb-6 leading-none">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {[
              ...
              <div
                key={idx}
                className="bg-gray-50 rounded-3xl p-8 transition-all duration-300 hover:bg-white hover:shadow-xl border border-gray-100 group"
              >
                <div className="w-12 h-12 bg-white shadow-sm text-gray-900 rounded-2xl flex items-center justify-center mb-6 group-hover:text-primary group-hover:scale-110 transition-all">
                  {card.icon}
                </div>

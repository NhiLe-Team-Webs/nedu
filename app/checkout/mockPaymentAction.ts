...
export async function processFreeCheckout(data: any) {
  try {
    ...
    const usageCount = rows.filter(r => r.get('Coupon Code') === 'LCMREVIEWFREE').length;
    
    if (usageCount >= 3) {
      return { success: false, message: 'Mã khuyến mãi này đã hết lượt sử dụng (Tối đa 3 lần).' };
    }

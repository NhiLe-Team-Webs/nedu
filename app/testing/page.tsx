...
import { testingCourses } from "@/data/testing-courses";
...
export default function TestingPage() {
    ...
    const navigateToCourse = (mode: string, slug: string) => {
        // For testing page, we don't have detail pages for mock courses
        alert("Đây là khóa học test, không có trang chi tiết. Vui lòng thêm vào giỏ hàng để test thanh toán.");
    };
    ...
    const filteredCourses = useMemo(() => {
        if (filter === "all") return testingCourses;
        if (filter === "offline") return testingCourses.filter((c) => c.mode === "offline");
        if (filter === "online") return testingCourses.filter((c) => c.mode === "online");
        return testingCourses.filter((c) => c.category.includes("Doanh nghiệp"));
    }, [filter]);

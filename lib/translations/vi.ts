export const vi = {
    common: {
        home: "TRANG CHỦ",
        about: "VỀ CHÚNG TÔI",
        courses: "KHÓA HỌC",
        challenge30: "THỬ THÁCH 30N",
        contact: "LIÊN HỆ",
        notifications: "Thông báo",
        cart: "Giỏ hàng",
        menu: "Menu",
        close: "Đóng",
    },
    header: {
        home: "TRANG CHỦ",
        about: "VỀ CHÚNG TÔI",
        courses: "KHÓA HỌC",
        challenge30: "THỬ THÁCH 30N",
        contact: "LIÊN HỆ",
    },
    footer: {
        home: "Trang chủ",
        about: "Về chúng tôi",
        courses: "Khóa học",
        contact: "Liên hệ",
        company_name: "CÔNG TY TNHH THƯƠNG MẠI DỊCH VỤ NHILE",
        tax_code: "Mã số thuế",
        address: "Lô 3, khu B2-7, Khu đô thị phức hợp Halla Jade Residence, Phường Hải Châu, TP Đà Nẵng, Việt Nam",
        payment_guide: "Hướng dẫn thanh toán",
        privacy_policy: "Chính sách bảo mật",
        terms_of_use: "Điều khoản sử dụng",
        payment_partner: "Đối tác thanh toán",
        rights_reserved: "All rights reserved",
    },
    bottom_tab: {
        home: "Trang chủ",
        courses: "Khóa học",
        challenge30: "30 Ngày",
        cart: "Giỏ hàng",
        menu: "Menu",
        extended_menu: "Menu mở rộng",
        contact: "Liên hệ",
        about: "Về chúng tôi",
        policy: "Chính sách",
        payment: "Thanh toán",
        close: "Đóng",
    },
    home: {
        hero_text_1: "LÀ CHÍNH MÌNH",
        hero_text_2: "THẤU HIỂU BẢN THÂN",
        hero_text_3: "QUẢN TRỊ CẢM XÚC",
        hero_subtitle_start: "\"Hành trình vạn dặm bắt đầu từ một bước chân được...",
        hero_subtitle_end: "...cùng N-Education\""
    },
    courses: {
        heading: "Người Việt, làm hàng Việt, cho người Việt",
        upcoming: "Khóa học sắp diễn ra",
        intro: "Giới thiệu",
        online: "Các khóa học Online",
        slides: {
            lcm: {
                title: "Là Chính Mình 04",
                content: "Người Việt, làm hàng Việt, cho người Việt."
            },
            smvh: {
                title: "Sức Mạnh Vô Hạn",
                content: "Khám phá bản thân và phát triển cá nhân."
            },
            thcb: {
                title: "Thương Hiệu Của Bạn",
                content: "Bắt đầu sự nghiệp với AI từ con số 0."
            },
            cscb: {
                title: "Cuộc Sống Của Bạn",
                content: "Xây dựng thương hiệu cá nhân và doanh nghiệp."
            },
            ai_biz: {
                title: "AI For Business Communication",
                content: "Quản lý cảm xúc và cân bằng cuộc sống."
            },
            ai_mkt: {
                title: "AI In Marketing",
                content: "Ứng dụng AI vào vận hành và kinh doanh."
            }
        }
    },
    mission: {
        heading: "Sứ mệnh",
        title: "VỀ CHƯƠNG TRÌNH",
        subtitle: "SỨ MỆNH",
        description: "Chúng tôi tin rằng mỗi người đều có một sức mạnh vô hạn bên trong. Sứ mệnh của chúng tôi là đánh thức sức mạnh đó.",
        quote: "Mang kiến thức, giáo dục chất lượng trên toàn thế giới về Việt Nam và hòa hợp với văn hóa người Việt",
        learn_more: "xem thêm",
        role: "Doanh nhân"
    },
    organizers: {
        heading: "Đơn vị tổ chức và vận hành"
    },
    support: {
        heading: "Bạn đang cần tư vấn thêm gì?",
        subheading: "HÃY ĐỂ N-EDU HỖ TRỢ CHO BẠN",
        placeholder: "Nhập email của bạn..."
    },
    connection: {
        heading: "KẾT NỐI CÙNG N-EDU",
        subheading: "ĐỂ CẬP NHẬT THÔNG TIN MỚI NHẤT TỪ CÁC KHÓA HỌC"
    },
    privilege: {
        heading: "Đặc Quyền Học Viên",
        subheading: "Nhận được sự hỗ trợ toàn diện và những quyền lợi độc quyền khi tham gia hành trình cùng N-Education.",
        items: {
            community: {
                title: "Tham gia cộng đồng N-Edu",
                description: "Kết nối với những người cùng chí hướng, sẵn lòng đồng hành và hỗ trợ lẫn nhau."
            },
            lifetime: {
                title: "Học lại trọn đời",
                description: "Miễn phí tham gia lại các khóa học đã tốt nghiệp, luôn được cập nhật kiến thức mới nhất."
            },
            support: {
                title: "Hỗ trợ sau khi học",
                description: "Đội ngũ giảng viên và chuyên gia luôn sẵn sàng giải đáp thắc mắc và hỗ trợ bạn."
            },
            global: {
                title: "Hợp tác quốc tế",
                description: "Cơ hội kết nối và học hỏi từ các chuyên gia và đối tác trên toàn cầu."
            },
            personal: {
                title: "Cá nhân hóa lộ trình",
                description: "Được tư vấn và xây dựng lộ trình phát triển phù hợp với mục tiêu cá nhân của bạn."
            },
            team: {
                title: "Đội ngũ hỗ trợ liên tục",
                description: "Nhận được sự đồng hành và hỗ trợ 24/7 từ đội ngũ chuyên nghiệp của N-Education."
            }
        }
    },
    banner: {
        marquee: {
            early_bird: "⚡️ EARLY BIRD GIẢM 10.000.000Đ - CHỈ CÒN 20 SUẤT",
            date_range: "⚡️ ƯU ĐÃI TỪ 08/12/2025 ĐẾN 08/01/2026",
            register_now: "⚡️ ĐĂNG KÝ NGAY ĐỂ GIỮ CHỖ",
            ends_in: "⚡️ ƯU ĐÃI KẾT THÚC SAU",
            days: "NGÀY"
        },
        main: {
            expires_in: "Ưu đãi sắp hết hạn sau:",
            days: "Ngày",
            hours: "Giờ",
            minutes: "Phút",
            seconds: "Giây"
        }
    },
    sidebar: {
        more: "Xem thêm",
        call: "Gọi ngay",
        email: "Email",
        chat: "Chat hỗ trợ",
        support_menu: "Menu hỗ trợ"
    },
    cart_popup: {
        added: "Đã thêm vào giỏ hàng",
        success: "Khóa học đã được thêm thành công"
    },
    testimonials: {
        heading: "CẢM NHẬN HỌC VIÊN",
        see_more: "Xem thêm hành trình",
        captions: {
            0: "Chia sẻ từ học viên",
            1: "Câu chuyện thay đổi",
            2: "Cảm nhận khóa học"
        }
    },
    partners: {
        background_text: "Partners",
        heading: "30+ đối tác"
    },
    top_banner: {
        desktop: {
            offer: "ƯU ĐÃI 20 SLOTS SỚM NHẤT: NHẬP MÃ",
            discount: "GIẢM NGAY 10.000.000Đ LỚP LÀ CHÍNH MÌNH",
            button: "Đăng ký ngay"
        },
        mobile: {
            offer: "ƯU ĐÃI EARLY BIRD GIẢM 10.000.000Đ",
            code_label: "NHẬP MÃ:",
            slots_left: "CHỈ CÒN 20 SUẤT",
            aria_label: "Chi tiết ưu đãi"
        }
    },
    notification_modal: {
        title: "ƯU ĐÃI ĐẶC BIỆT",
        now: "Ngay bây giờ",
        close: "Đóng",
        discount_prefix: "Giảm ngay",
        discount_suffix: "cho khóa học \"Là Chính Mình #04\"",
        code_label: "Mã ưu đãi",
        copied: "Đã copy",
        copy: "Copy",
        button: "Đăng ký ngay",
        footer: "Ưu đãi có giới hạn. Áp dụng cho 20 người đăng ký sớm nhất."
    },
    sepay: {
        success: {
            title: "Thanh toán thành công!",
            message: "Đơn hàng của bạn đã được xác nhận thanh toán thành công.",
            continue: "Tiếp tục ngay",
            redirect_hint: "Tự động chuyển trang sau 3 giây..."
        },
        failed: {
            title: "Thanh toán thất bại",
            message: "Đã xảy ra lỗi trong quá trình thanh toán. Vui lòng thử lại."
        },
        scan_qr: {
            title: "Quét mã QR để thanh toán",
            instruction: "Sử dụng ứng dụng ngân hàng của bạn để quét mã QR và hoàn tất thanh toán",
            loading: "Đang tải QR code..."
        },
        info: {
            order_code: "Mã đơn hàng:",
            amount: "Số tiền:",
            account_number: "Số tài khoản:",
            bank: "Ngân hàng:"
        },
        status: {
            checking: "Đang kiểm tra thanh toán...",
            waiting: "Đang chờ thanh toán...",
            auto_check: "Hệ thống sẽ tự động kiểm tra thanh toán sau khi bạn hoàn tất chuyển khoản"
        }
    },
    challenge_register: {
        scan_title: "Quét mã thanh toán",
        scan_desc: "Vui lòng quét mã QR bên dưới để hoàn tất đăng ký.<br />Hệ thống sẽ tự động xác nhận khi nhận được tiền.",
        beneficiary: "Người thụ hưởng:",
        content_ck: "Nội dung CK:",
        copy_hint: "(*Bấm vào nội dung để sao chép)",
        waiting_confirm: "Đang chờ xác nhận thanh toán...",
        form_title: "ĐĂNG KÝ THAM GIA",
        labels: {
            name: "Họ và tên",
            email: "Email",
            phone: "Số điện thoại *",
            telegram: "Username Telegram *",
            dob: "Ngày sinh *",
            gender: "Giới tính *",
            address: "Địa chỉ",
            note: "Ghi chú"
        },
        placeholders: {
            name: "Nhập họ tên của bạn",
            email: "example@gmail.com",
            phone: "Nhập số điện thoại (Zalo)",
            telegram: "@username",
            select_gender: "Chọn giới tính",
            address: "Nhập địa chỉ của bạn",
            note: "Nhập ghi chú (nếu có)"
        },
        gender_options: {
            male: "Nam",
            female: "Nữ",
            other: "Khác"
        },
        buttons: {
            processing: "Đang xử lý...",
            pay: "THANH TOÁN (396.000đ)"
        },
        footer: "Thông tin thanh toán sẽ được gửi và tích hợp tự động."
    },
    categories: {
        entrepreneur: "Doanh nhân",
        business: "Doanh nghiệp",
        personal_development: "Phát triển bản thân",
        be_yourself: "Là chính mình",
        ai: "AI",
        branding: "Thương hiệu",
        applied_ai: "Trí tuệ nhân tạo ứng dụng",
        digital_marketing: "Marketing số"
    },
    program_page: {
        title: "KHÓA HỌC",
        subtitle: "Lựa chọn chương trình phù hợp với hành trình phát triển bản thân và doanh nghiệp của bạn. Mỗi khóa học đều đi kèm công cụ thực hành và có thể tham gia ngay.",
        filters: {
            all: "Tất cả",
            offline: "Offline",
            online: "Online",
            business: "Doanh nghiệp"
        },
        card: {
            sessions: "Số buổi học",
            instructor: "Người dẫn đường",
            add_to_cart: "Thêm vào giỏ hàng",
            added: "Đã thêm vào giỏ hàng",
            learn_more: "Tìm hiểu thêm",
            register: "Đăng ký",
            previous: "Trước",
            next: "Tiếp"
        },
        toast: {
            added_to_cart: "đã được thêm vào giỏ hàng",
            max_quantity_reached: "Bạn đã thêm tối đa 10 khóa học này vào giỏ hàng"
        }
    },
    program_detail: {
        info: {
            title: "THÔNG TIN KHÓA HỌC",
            topic: "Chủ đề:",
            schedule: "Thời gian học:",
            instructor: "Người dẫn đường:",
            sessions: "Số buổi học:",
            location: "Địa điểm học:",
            capacity: "Số lượng học viên:"
        },
        common: {
            why_title: "Tại sao N-Education?",
            online_course: "Khóa học Online",
            offline_course: "Khóa học Offline",
            who_should_join: "Ai nên tham gia khóa học này?",
            add_to_cart: "Thêm vào giỏ hàng",
            testimonials_title: "Testimonials",
            testimonials_subtitle: "Lời chứng thực",
            why_subtitle_1: "Nơi duy nhất để bạn",
            why_subtitle_2: "LÀ CHÍNH MÌNH",
            roadmap_title: "Lộ trình.",
            roadmap_subtitle: "Dành riêng cho bạn.",
            roadmap_desc: "3.5 ngày tháo gỡ, chữa lành và tái thiết kế cuộc đời.",
            day: "Day",
            outcome: "Giá trị nhận được",
            outcome_label: "OUTCOME",
            whom_title: "Ai nên tham gia khóa học này?",
            cost_label: "Chi phí:",
        },
        courses: {
            la_chinh_minh: {
                title: "Là Chính Mình 04",
                mission_desc: "Là Chính Mình - Hành trình 3.5 ngày đánh thức sức mạnh nội tại, giúp bạn gỡ bỏ những rào cản tâm lý để sống một cuộc đời rực rỡ và trọn vẹn nhất.",
                topic: "Phát triển bản thân và khám phá nội tâm",
                schedule: "05–08/03/2026",
                instructor: "NhiLe x Guest Instructors",
                sessions: "3,5 ngày",
                location: "Đà Nẵng",
                capacity: "64 học viên",
                days: [
                    {
                        title: "AWAKEN",
                        sub: "Tỉnh thức",
                        hook: "Đánh thức ngọn lửa bên trong.",
                        desc: "Ngừng sống mòn. Chúng ta sẽ đi xuyên qua những tầng lớp của sự sợ hãi để chạm vào động lực gốc rễ.",
                        bullets: [
                            "Khai phá 'Big Why' – Lý do tồn tại.",
                            "Gỡ bỏ kỳ vọng vay mượn.",
                            "Đối thoại: Bạn là ai?"
                        ],
                        activities: ["Game: Simulation Life", "Vòng tròn Sự thật"],
                        outcome: "Sự sáng rõ. Biết chính xác mình sống vì điều gì."
                    },
                    {
                        title: "UNBOX",
                        sub: "Khai phóng",
                        hook: "Phá vỡ những chiếc hộp vô hình.",
                        desc: "Tiền bạc. Định kiến. Môi trường. Những rào cản vô hình đang giam cầm tiềm năng của bạn sẽ bị đập tan.",
                        bullets: [
                            "Giải mã tư duy tài chính.",
                            "Thanh lọc quan hệ độc hại.",
                            "Cài đặt tư duy làm chủ."
                        ],
                        activities: ["Bản đồ Tài chính", "Game: Hệ sinh thái"],
                        outcome: "Sự tự do. Nắm lại quyền kiểm soát vận mệnh."
                    },
                    {
                        title: "REVEAL",
                        sub: "Chữa lành",
                        hook: "Đối diện để thấy bình yên.",
                        desc: "Gỡ bỏ lớp mặt nạ 'mạnh mẽ' giả tạo để ôm lấy tổn thương. Chỉ khi dám đối diện, bạn mới được chữa lành.",
                        bullets: [
                            "Đối mặt với 'Bóng tối' (Shadow Work).",
                            "Kết nối đứa trẻ bên trong.",
                            "Chạm vào cốt lõi linh hồn."
                        ],
                        activities: ["Mirror: Đối diện", "Lễ lột xác (Signature)"],
                        outcome: "Sự nhẹ nhõm. Hạnh phúc vì được là chính mình."
                    },
                    {
                        title: "REBORN",
                        sub: "Tái sinh",
                        hook: "Sống là chính mình.",
                        desc: "Bước ra thế giới với một nhân dạng mới. Kiên cường. Bản lĩnh. Và hoàn toàn chân thật.",
                        bullets: [
                            "Hợp nhất nội tâm.",
                            "Lộ trình 5 năm di sản.",
                            "Cam kết sống không hối tiếc."
                        ],
                        activities: ["Mapping Future Self", "Ceremony Trở về"],
                        outcome: "Một bản thể rực rỡ. Sẵn sàng dẫn dắt cuộc đời."
                    }
                ],
                why_cards: [
                    {
                        title: "Khoa học Tâm thức",
                        desc: "Kết hợp Tâm lý học Hành vi & Khoa học não bộ. Tháo gỡ nỗi sợ từ gốc rễ cơ chế vận hành."
                    },
                    {
                        title: "Game Mô phỏng",
                        desc: "Không lý thuyết suông. Bạn học qua trải nghiệm, va vấp và vỡ òa ngay trong các Game giả lập cuộc đời."
                    },
                    {
                        title: "Không phán xét",
                        desc: "Một không gian an toàn tuyệt đối để trút bỏ mặt nạ, nơi sự yếu đuối của bạn được trân trọng và chữa lành."
                    },
                    {
                        title: "Kết quả Hữu hình",
                        desc: "Ra về với 'Bản thiết kế cuộc đời' 5 năm chi tiết. Biến nội lực thành hành động thực tế."
                    }
                ]
            },
            suc_manh_vo_han: {
                title: "Sức Mạnh Vô Hạn",
                topic: "Vận hành doanh nghiệp",
                schedule: "Tháng 4 năm 2026",
                instructor: "Mel x NhiLe",
                sessions: "6 tháng online và 4,5 ngày offline",
                location: "Online qua Zoom và Offline tại khách sạn Mikazuki Đà Nẵng",
                capacity: "40 học viên",
                mission_desc: "Sức Mạnh Vô Hạn được thiết kế để giúp doanh nhân Việt vươn tầm thế giới. Bằng cách kết nối với các Chuyên gia quốc tế, bạn sẽ học những chiến lược đột phá và xây dựng những nền tảng căn bản nhất cho doanh nghiệp, nâng cấp khả năng kinh doanh và mở rộng thị trường.",
                testimonials: [
                    "Lần đầu Trang tham gia một chương trình của người Việt mà ấn tượng đến vậy. Cường độ học tập áp lực như môi trường doanh nhân thật sự.",
                    "Bằng cấp kinh tế không đảm bảo bạn trở thành doanh nhân thành công. Muốn đi xa, bạn cần hơn cả kiến thức – đó là tư duy, chiến lược, đội nhóm phù hợp với chính mình.",
                    "Cần ít nhất 3 năm đi vô đi ra lại cái lớp như vậy để các bạn có thể bắt đầu hiểu vấn đề và bạn thay đổi học phát triển bản thân"
                ]
            },
            ai_for_business_communication: {
                title: "AI For Business Communication",
                description: "Khóa học chuyên sâu về ứng dụng AI trong giao tiếp kinh doanh, giúp các chuyên gia và doanh nhân tối ưu hóa kỹ năng truyền thông trong kỷ nguyên số.",
                topic: "AI For Business Communication",
                schedule: "28-29 tháng 7 năm 2025",
                instructor: "Linda Hui-Isaac",
                sessions: "2 buổi",
                location: "Online qua Zoom",
                capacity: "40 học viên / lớp"
            },
            ai_in_marketing: {
                title: "AI In Marketing",
                description: "Khóa học chuyên sâu về ứng dụng AI trong marketing, giúp các marketer hiện đại hóa chiến lược và tối ưu hóa hiệu quả chiến dịch.",
                topic: "AI In Marketing",
                schedule: "28-29 tháng 7 năm 2025",
                instructor: "Linda Hui-Isaac",
                sessions: "2 buổi",
                location: "Online qua Zoom",
                capacity: "40 học viên / lớp"
            },
            cuoc_song_cua_ban: {
                title: "Cuộc Sống Của Bạn",
                description: "Khóa học giúp bạn khám phá và định hình lại cuộc sống theo cách riêng, tìm ra con đường phát triển bản thân và xây dựng lối sống ý nghĩa.",
                topic: "Phát triển bản thân",
                schedule: "10-13/09/2025",
                instructor: "NhiLe",
                sessions: "3,5 ngày",
                location: "Online qua Zoom",
                capacity: "Giới hạn",
                testimonials: [
                    "Tâm cơ cực kỳ hào hứng để có thể chia sẻ với tất cả các bạn những cái trải nghiệm vừa rồi về 5 bài test thông dụng để xác định một người nào đó có những cái đặc tính nào nổi trội",
                    "Nhưng mà trước khi mà các bạn muốn yêu một ai đó một cách đúng ấy thì các bạn phải yêu bản thân mình một cách đúng trước đã",
                    "Một cái khá là hay trong cái khóa học lần này là Nhi Lê cho các bạn những cái bài test những cái câu hỏi những cái công cụ để các bạn có thể xác định được cái chỉ số của mình"
                ]
            },
            gen_ai_101: {
                title: "GEN AI 101",
                description: "Giúp người Việt Nam hiểu rõ công nghệ Gen AI và ứng dụng thực tế vào công việc một cách có trách nhiệm và hiệu quả. Đồng thời, tạo nền tảng để tối ưu hoá các thao tác công việc lặp đi lặp lại hàng ngày trong công việc, đặc biệt trong lĩnh vực công và giáo dục.",
                topic: "AI",
                schedule: "28-29 tháng 7 năm 2025",
                instructor: "Linda Hui-Isaac",
                sessions: "2 buổi",
                location: "Online qua Zoom",
                capacity: "40 học viên / lớp"
            },
            thuong_hieu_cua_ban: {
                title: "Thương Hiệu Của Bạn",
                description: "Kiến thức cơ bản nhất cho người muốn bắt đầu mở doanh nghiệp hay cải tổ doanh nghiệp gia đình. Định hình rõ hơn kinh doanh của bạn trong chỉ 4 ngày với khóa học Thương Hiệu Của Bạn - một chương trình được thiết kế đặc biệt cho chủ doanh nghiệp trong kỷ nguyên AI.",
                topic: "Thương hiệu",
                schedule: "10-13/09/2025",
                instructor: "NhiLe",
                sessions: "3,5 ngày",
                location: "Online qua Zoom",
                capacity: "Giới hạn"
            }
        },
        whom: {
            student_newbie: {
                heading: "Sinh viên & người mới đi làm",
                description: "Trang bị nền tảng thực tế để hiểu rõ AI dùng trong công việc viết – trình bày – giao tiếp, mở ra cơ hội phát triển sớm trong môi trường chuyên nghiệp."
            },
            career_switcher: {
                heading: "Người muốn chuyển ngành",
                description: "Khóa học phù hợp cho những ai muốn khám phá tiềm năng AI để ứng dụng trong công việc mới như marketing, truyền thông, quản lý, nhân sự..."
            },
            ai_beginner: {
                heading: "Người mới bắt đầu học AI",
                description: "Không cần nền tảng kỹ thuật – chỉ cần bạn tò mò, ham học hỏi và muốn dùng AI một cách thực tế để nâng cấp kỹ năng và tăng năng suất cá nhân."
            }
        },
        instructors: {
            nhi_le: {
                profession: ["Doanh nhân"],
                bio: "Nhi Le là cố vấn tâm lý và doanh nhân, đồng hành cùng nhiều người Việt trong hành trình hiểu bản thân, xây dựng nội lực và tạo ra thay đổi bền vững.",
                education: [
                    "Nhi Le có nền tảng học thuật bài bản trong tâm lý học, coaching và lãnh đạo, với các chứng chỉ chuyên môn từ Kaplan Singapore và Singapore Management University (SMU), bao gồm tâm lý học, coaching doanh nghiệp, lãnh đạo bền vững và hành vi học ứng dụng."
                ],
                career: "Sinh ra và lớn lên tại Quảng Nam, hiện là mẹ đơn thân sống và làm việc ở nước ngoài, Nhi Le mang đến góc nhìn rất thực tế về tự lập, kỷ luật cá nhân và trách nhiệm với chính cuộc sống của mình. Chị không dạy lý thuyết suông, mà dẫn dắt bằng trải nghiệm thật và quá trình chuyển hóa thật.",
                achievements: [
                    { date: "2025-04", description: "Vinh danh với giải thưởng HER Courage Awards 2025." },
                    { date: "2025-08", description: "Người phụ nữ Việt Nam đầu tiên xuất hiện trên blog chính thức của YouTube toàn cầu." }
                ],
                projects: {
                    youtube: { title: "Kênh YouTube Nhi Le", description: "Kênh tiên phong chia sẻ kiến thức tâm lý học bằng tiếng Việt." },
                    community: { title: "Cộng đồng NhiLe Team", description: "Đào tạo nghề và kỹ năng cần thiết cho giới trẻ." },
                    foundation: { title: "Quỹ NhiLe Foundation", description: "Tổ chức phi lợi nhuận hỗ trợ trẻ em Việt Nam." },
                    podcast: { title: "Podcast Ms Nhi và This is Home", description: "Kết nối thế hệ trẻ với doanh nhân, nghệ nhân Việt Nam." }
                }
            },
            mel: {
                profession: ["Chuyên gia Marketing"],
                bio: "Melvin Soh, chuyên gia marketing hàng đầu châu Á, nổi tiếng với hơn 15 năm kinh nghiệm thực chiến trong xây dựng thương hiệu và thu hút khách hàng. Anh đã giúp hàng trăm doanh nghiệp tạo dựng lòng trung thành và tăng trưởng bền vững.",
                education: "Thạc sĩ Quản trị Kinh doanh (MBA)",
                career: "CEO của Marketing Solutions Asia"
            },
            linda_hui_isaac: {
                profession: ["Doanh nhân"],
                bio: "Linda Hui-Isaac là một Chiến lược gia Lãnh đạo về Xây dựng Thương hiệu & AI-Marketing (Branding & AI-Marketing Consultant Lead Trainer). Bà có hơn hai thập kỷ kinh nghiệm toàn cầu, chuyên tư vấn và đào tạo về các chiến lược tiếp thị kỹ thuật số hiệu quả.",
                education: [
                    "- Thạc sĩ Quản trị Kinh doanh Toàn cầu (MBA in Global Business) tại Coventry University (Anh).",
                    "- Chứng chỉ Nâng cao về Học tập & Hiệu suất (Advanced Certificate in Learning & Performance) tại Institute of Adult Learning (IAL)."
                ],
                career: "Linda Hui-Isaac có hơn 20 năm kinh nghiệm trong lĩnh vực xây dựng thương hiệu và AI-Marketing, đã tư vấn cho nhiều công ty niêm yết trên SGX và các MNC thương hiệu xa xỉ Châu Âu.",
                achievements: [
                    { date: "2025", description: "Hơn 20 năm kinh nghiệm trong lĩnh vực Xây dựng Thương hiệu và AI-Marketing." },
                    { date: "2025", description: "Tích lũy kinh nghiệm từ các công ty niêm yết trên SGX và các MNC thương hiệu xa xỉ Châu Âu." },
                    { date: "2025", description: "Đào tạo các công ty lớn: AIA, DBS, UOB, SAXO." },
                    { date: "2025", description: "Đào tạo công nghệ: AWS, SONY, NBCUniversal, Ricoh." },
                    { date: "2025", description: "Đào tạo chính phủ: GOVTech, ICA Singapore, Ministry of Health (MOH)." },
                    { date: "2025", description: "Đào tạo giáo dục: NUS, NTU, SMU Academy, Kaplan." },
                    { date: "2025", description: "Đào tạo tư vấn: BCG." }
                ],
                projects: {
                    founder: { title: "Founder tại Crown Mercado Alliance Pte Ltd", description: "Bà đã tư vấn thành công cho nhiều Doanh nghiệp vừa và nhỏ (SME), Công ty đa quốc gia (MNC), và Cơ quan Chính phủ trong việc triển khai chiến lược tiếp thị kỹ thuật số." },
                    trainer: { title: "Huấn luyện viên Trưởng (Lead Trainer)", description: "Đã đào tạo hơn 5.000 học viên thuộc hơn 10 ngành nghề khác nhau. Các buổi đào tạo tập trung vào học tập thực hành và các chiến lược đổi mới." },
                    ai_strategy: { title: "Chuyên môn: Chiến lược Kinh doanh & Tiếp thị Tăng cường bởi AI", description: "Tập trung vào ứng dụng AI trong chiến lược kinh doanh và marketing." },
                    inbound: { title: "Chuyên môn: Inbound Omni-Channel", description: "Xây dựng chiến lược tiếp thị đa kênh theo hướng inbound." },
                    content: { title: "Chuyên môn: Nội dung, SEO, SEM & Social Media", description: "Bao gồm xây dựng nội dung, tối ưu hóa tìm kiếm, quảng cáo và mạng xã hội." },
                    training: { title: "Chủ đề đào tạo", description: "AI trong Marketing, Quảng cáo TikTok." }
                }
            }
        },
        categories: {
            entrepreneur: "Doanh nhân",
            business: "Doanh nghiệp",
            personal_development: "Phát triển bản thân",
            be_yourself: "Là chính mình",
            ai: "AI",
            branding: "Thương hiệu",
            emotion: "Cảm xúc",
            applied_ai: "Trí tuệ nhân tạo ứng dụng",
            digital_marketing: "Marketing số"
        },
        instructor_component: {
            heading: "Người Dẫn Đường",
            education: "Học vấn",
            career: "Sự nghiệp và các dự án nổi bật",
            achievements: "Thành tích và giải thưởng",
            profession: "Nghề nghiệp"
        }
    },
    thirty_day_challenge: {
        title: "THỬ THÁCH 30 NGÀY",
        title_membership: "THỬ THÁCH 30 NGÀY - GÓI MEMBERSHIP",
        topic: "Chủ đề Tháng 12:",
        topic_name: "Tỏa sáng cùng Denise",
        cost_label: "Chi phí mỗi tháng:",
        register_now: "ĐĂNG KÝ NGAY",
        about_title: "Nhi đây!",
        about_content: "Có ai học kiến thức của Nhi xong bỏ vô góc trì hoãn không làm không? Thử Thách 30 Ngày ra đời để kéo bạn vào hành trình hành động cùng nhau, từng bước xoá thói quen xấu.",
        about_content_2: "Năm 2025, Nhi muốn gần hơn với khán giả của mình, cùng các bạn tạo ra nhiều điều tốt đẹp hơn cho cuộc sống!",
        what_is_section: {
            heading: "30 NGÀY CÓ GÌ?",
            col_1: {
                title: "Thử thách 30 ngày là gì?",
                content: "Đây là hành trình 30 ngày hành động liên tục dưới sự quan sát và thúc đẩy của Nhi Lê, ngoài ra còn được hỗ trợ 24/7 từ đội ngũ chuyên nghiệp.\n\nMỗi ngày, bạn chỉ cần dành 5–10 phút để thực hiện một nhiệm vụ đơn giản và check-in trong nhóm.\n\nTừng bước nhỏ, nhưng đủ để bạn cảm nhận mình đang thay đổi."
            },
            col_2: {
                title: "Ai nên tham gia?",
                items: [
                    "Những người đang mất động lực, trì hoãn và muốn bắt đầu lại.",
                    "Người muốn xây dựng thói quen, kỹ năng và năng lượng sống tốt hơn.",
                    "Người muốn ở trong một môi trường tích cực để phát triển bản thân."
                ]
            }
        },
        mentor: {
            bg_text: "NGƯỜI HƯỚNG DẪN",
            title: "Người Dẫn Đường",
            role: "Host & Content Creator",
            desc_1: "Host & Content Creator, tập trung vào Phát triển cá nhân cho những người không nghiêm túc.",
            desc_2: "Truyền cảm hứng cho hơn 23.000 người trên toàn thế giới qua podcast, YouTube và những cuộc trò chuyện chân thật.",
            passion_label: "Đam mê với việc thay đổi nhận thức:",
            passion_content: "Giúp bạn nhận ra không cần thay đổi tính cách để đạt được điều tuyệt vời — hãy phát triển bằng cách là chính mình."
        },
        faq: {
            heading: "Câu hỏi thường gặp",
            description: "Giải đáp những thắc mắc phổ biến nhất về các chương trình đào tạo tại N-Education.",
            contact_button: "Liên hệ tư vấn viên",
            see_all: "Xem tất cả",
            collapse: "Thu gọn",
            items: [
                {
                    question: "Không là thành viên của NhiLe Team thì có tham gia được không?",
                    answer: "Hoàn toàn được nhé 💛 30 Days mở cho tất cả mọi người. Bạn không cần là thành viên của NhiLe Team từ trước, chỉ cần mong muốn thay đổi và sẵn sàng bắt đầu là đủ."
                },
                {
                    question: "Người mới, chưa từng tham gia thử thách bao giờ có phù hợp không?",
                    answer: "Rất phù hợp. 30 Days được thiết kế cho cả người mới bắt đầu, mọi nhiệm vụ đều đơn giản, dễ hiểu và có hướng dẫn rõ ràng."
                },
                {
                    question: "Mình bận, không online thường xuyên thì có theo được không?",
                    answer: "Có thể theo được nha. Mỗi ngày bạn chỉ cần dành khoảng 5–10 phút, không cần online liên tục hay lúc nào cũng phải có mặt. Nhưng bạn ơi, Admin khuyến khích bạn hãy tích cực hoạt động và trò chuyện với mọi người trong nhóm. Đây cũng là một cách để phát triển bản thân nhé ^^"
                },
                {
                    question: "Có giới hạn độ tuổi hoặc ngành nghề không?",
                    answer: "Không có giới hạn nào cả. Dù bạn đang đi học, đi làm hay ở giai đoạn nào trong cuộc sống, nếu thấy phù hợp thì đều có thể tham gia."
                },
                {
                    question: "Các chủ đề năm 2026 có giống năm 2025 không?",
                    answer: "Trong 2026 sẽ có cả những chủ đề mới và một số chủ đề quen thuộc quay lại.\\nTuy nhiên, 30 Days luôn được thiết kế như một hành trình nhẹ nhàng để trải nghiệm và làm mới bản thân, nên mỗi lần tham gia, bạn sẽ cảm nhận được những góc nhìn khác nhau."
                },
                {
                    question: "Nếu đã tham gia 30 Days trước đây thì có nên tham gia lại không?",
                    answer: "Rất nên nha 💛 Nhiều bạn quay lại 30 Days để đào sâu hơn, giữ nhịp và bứt phá ở tầng mới.\\nVà nếu bạn cảm thấy mình muốn đi sâu hơn nữa, bạn có thể tham khảo thêm các chương trình chuyên sâu của Nhi Lê để tiếp tục hành trình phát triển bản thân."
                },
                {
                    question: "Mỗi ngày mình phải làm gì và mất bao lâu?",
                    answer: "Mỗi ngày bạn sẽ nhận một nhiệm vụ nhỏ, dễ thực hiện, chỉ mất khoảng 5–10 phút để hoàn thành và check-in trong nhóm."
                },
                {
                    question: "Nhóm có hoạt động nhiều không hay chỉ nhận nhiệm vụ?",
                    answer: "Nhóm hoạt động khá đều, nhưng rất nhẹ nhàng. Bạn có thể chia sẻ, đọc câu chuyện của người khác hoặc chỉ check-in ngắn gọn, tuỳ theo mức thoải mái của bạn."
                },
                {
                    question: "Mình có cần chia sẻ nhiều thông tin cá nhân không?",
                    answer: "Không cần đâu. Khi đăng ký, bạn chỉ cần điền những thông tin cần thiết để tham gia chương trình.\\nNếu trong quá trình tổ chức có cần thêm thông tin, admin sẽ luôn nhắn tin trao đổi trước với bạn, không tự ý thu thập hay sử dụng ngoài mục đích chương trình."
                },
                {
                    question: "Nếu mình hướng nội, ít nói thì có phù hợp không?",
                    answer: "Rất phù hợp. 30 Days không yêu cầu bạn phải nói nhiều hay thể hiện liên tục, chỉ cần bạn hiện diện và làm theo nhịp của mình."
                },
                {
                    question: "Có ai nhắc nhở nếu mình quên làm nhiệm vụ không?",
                    answer: "Có nhé. Group Telegram luôn có nhắc nhở nhẹ nhàng để bạn không bị quên hay bỏ lỡ quá nhiều ngày."
                },
                {
                    question: "Nên chọn gói theo tháng hay Membership 12 tháng?",
                    answer: "Gói theo tháng phù hợp nếu bạn muốn thử trải nghiệm.\\nMembership 12 tháng dành cho những bạn muốn đồng hành lâu dài và tham gia trọn vẹn các chủ đề trong năm."
                },
                {
                    question: "Nếu tham gia giữa chừng thì có được không?",
                    answer: "30 Days chỉ mở đăng ký trong thời gian nhất định thôi.\\nChương trình khuyến khích bạn tham gia từ đầu hành trình, vì khi đi cùng nhau trọn vẹn 30 ngày thì trải nghiệm sẽ vui hơn, kết nối tốt hơn và hiệu quả cũng rõ ràng hơn.\\nVì vậy, chương trình không nhận tham gia giữa chừng để đảm bảo chất lượng cho tất cả mọi người."
                },
                {
                    question: "Membership có thể bảo lưu nếu bận không?",
                    answer: "Có. Trong trường hợp bạn bận hoặc cần nghỉ một thời gian, Membership sẽ được hỗ trợ bảo lưu theo chính sách của chương trình."
                }
            ]
        },
        info: {
            title: "THÔNG TIN KHÓA HỌC",
            topic: "Chủ đề",
            time: "Thời gian",
            instructor: "Người dẫn đường",
            format: "Hình thức",
            format_content: "Thực hiện thử nghiệm & báo cáo nhóm",
            register: "ĐĂNG KÝ THAM GIA NGAY"
        },
        timeline: {
            heading: "SAU 30 NGÀY BẠN ĐƯỢC...?",
            block_1: {
                title: "Nắm rõ thu và chi mỗi tháng",
                quote: "“Tiền không khó quản lý, khó là mình không nhìn rõ nó.”",
                collapsed: "NẮM RÕ\\nTHU VÀ CHI",
                content: "Sau 30 ngày, bạn sẽ biết rõ mỗi tháng tiền đi đâu, số dư ra sao. Không còn 'sao tiền hết nhanh thế', không còn cảm giác mơ hồ – bạn nắm được bức tranh tài chính cá nhân thật sự."
            },
            block_2: {
                title: "Chi tiêu có kiểm soát",
                quote: "“Xài tiền không sai, xài mà không biết vì sao mới mệt.”",
                collapsed: "CHI TIÊU\\nCÓ KIỂM SOÁT",
                content: "Bạn sẽ bắt đầu suy nghĩ trước khi chi. Không phải kiểm soát quá mức, nhưng bạn đã chuyển từ chi tiêu vô thức sang chi tiêu có chủ đích, rõ ràng với từng đồng tiền ra – vào."
            },
            block_3: {
                title: "Xây dựng thói quen chi tiêu lành mạnh",
                quote: "“Thứ làm bạn mệt không phải tiền, mà là cách xài tiền.”",
                collapsed: "XÂY DỰNG\\nTHÓI QUEN\\nCHI TIÊU\\nLÀNH MẠNH",
                content: "Sau 30 ngày, bạn hình thành được thói quen quản lý tiền rõ ràng và thực tế, đủ để duy trì lâu dài. Tiền bạc trở nên gọn gàng hơn, áp lực tài chính giảm đi, và bạn chủ động hơn trong các quyết định chi tiêu hàng ngày."
            },
            time_label: "THỜI GIAN DIỄN RA",
            students_label: "SỐ LƯỢNG HỌC VIÊN",
            students_count: "99 người",
            location_label: "ĐỊA ĐIỂM",
            location_value: "Online qua Zoom"
        },
        benefits: {
            heading: "ĐẶC QUYỀN HỌC VIÊN",
            items: {
                community: { title: "Tham gia cộng đồng N-Edu", description: "Kết nối với những người cùng chí hướng, sẵn lòng đồng hành và hỗ trợ lẫn nhau." },
                global: { title: "Hợp tác quốc tế", description: "Cơ hội kết nối và học hỏi từ các chuyên gia và đối tác trên toàn cầu." },
                personalized: { title: "Cá nhân hóa lộ trình", description: "Được tư vấn và xây dựng lộ trình phát triển phù hợp với mục tiêu cá nhân của bạn." },
                support: { title: "Hỗ trợ liên tục", description: "Nhận được sự đồng hành và hỗ trợ 24/7 từ đội ngũ chuyên nghiệp của N-Education." }
            }
        },
        instructor_section: {
            heading: "NGƯỜI DẪN ĐƯỜNG",
            profession_label: "NGHỀ NGHIỆP",
            profession: "Doanh nhân",
            bio_1: "Nhi Le là cố vấn tâm lý và doanh nhân, đồng hành cùng nhiều người Việt trong hành trình hiểu bản thân, xây dựng nội lực và tạo ra thay đổi bền vững.",
            bio_2: "Sinh ra và lớn lên tại Quảng Nam, hiện là mẹ đơn thân sống và làm việc ở nước ngoài, Nhi Le mang đến góc nhìn rất thực tế về tự lập, kỷ luật cá nhân và trách nhiệm với chính cuộc sống của mình. Chị không dạy lý thuyết suông, mà dẫn dắt bằng trải nghiệm thật và quá trình chuyển hóa thật.",
            bio_3: "Nhi Le có nền tảng học thuật bài bản trong tâm lý học, coaching và lãnh đạo, với các chứng chỉ chuyên môn từ Kaplan Singapore và Singapore Management University (SMU), bao gồm tâm lý học, coaching doanh nghiệp, lãnh đạo bền vững và hành vi học ứng dụng."
        },
        pricing: {
            heading: "LỰA CHỌN GÓI HỌC",
            subtitle: "Chọn gói học phù hợp nhất với nhu cầu và mục tiêu của bạn để bắt đầu hành trình chuyển hóa.",
            monthly: {
                label: "Gói Theo Tháng",
                per_month: "/ tháng",
                button: "Đăng ký ngay",
                added: "Đã thêm vào giỏ hàng",
                features: {
                    topic: "Tham gia 01 chủ đề 30 Days trong tháng đăng ký",
                    daily_activities: "30 hoạt động mỗi ngày (micro-habits)",
                    zoom: "1 buổi Zoom cùng Host (Nếu có)",
                    telegram: "Tham gia nhóm Telegram của tháng đó",
                    support: "Hỗ trợ trong 30 ngày",
                    no_archive: "Không truy cập kho nội dung cũ",
                    no_bonus: "Không bao gồm bonus hoặc workshop thêm"
                }
            },
            membership: {
                label: "Gói Membership",
                per_year: "/ 12 tháng",
                badge: "KHUYÊN DÙNG",
                button: "Chọn Gói Membership",
                added: "Đã thêm vào giỏ hàng",
                features: {
                    all_topics: "Truy cập toàn bộ 12 chủ đề 30 Days trong 1 năm",
                    all_challenges: "Tham gia tất cả các thử thách mà không cần mua thêm",
                    archive_access: "Truy cập kho tài liệu & thử thách cũ",
                    bonus_workshop: "Mini workshop bonus dành riêng cho thành viên",
                    member_group: "Gia nhập Group Member (duy trì suốt năm, không reset mỗi tháng)",
                    year_support: "Hỗ trợ xuyên suốt 12 tháng từ đội ngũ",
                    pause_right: "Quyền bảo lưu tối đa 1–2 tháng nếu bận",
                    advanced_discount: "Ưu đãi khi tham gia các chương trình nâng cao của Nhi Lê"
                }
            },
            features: {
                zoom: "Tham gia Zoom mỗi tháng",
                community: "Tham gia cộng đồng học viên",
                qa: "Hỗ trợ Q&A qua Telegram",
                review: "Xem lại bài học",
                all_topics: "Tham gia tất cả chủ đề trong năm",
                early_access: "Ưu tiên đăng ký các khóa học mới"
            }
        }
    },
    contact: {
        title: "Kết nối với chúng tôi",
        subtitle: "Điền đầy đủ thông tin để nhận tư vấn từ N-EDU",
        form: {
            name: "Họ và tên",
            phone: "Số điện thoại",
            email: "Email",
            content: "Nội dung",
            submit: "GỬI",
            success: "Đã gửi thành công!",
            placeholders: {
                name: "Nhập họ và tên của bạn",
                phone: "Nhập số điện thoại",
                email: "Nhập email của bạn",
                content: "Bạn cần hỗ trợ hay điều gì?"
            }
        },
        sidebar: {
            contact: "Liên hệ",
            email: "Email",
            telegram: "Telegram"
        }
    },
    terms: {
        title: "Điều khoản sử dụng",
        sections: {
            intro: {
                title: "Chấp nhận điều khoản",
                content: "Bằng cách truy cập và sử dụng website và dịch vụ của N-Edu, bạn đồng ý tuân thủ các điều khoản và điều kiện này. Nếu bạn không đồng ý với bất kỳ phần nào của điều khoản, vui lòng không sử dụng dịch vụ của chúng tôi."
            },
            definitions: {
                title: "Định nghĩa",
                items: [
                    { label: '"N-Edu" hoặc "Chúng tôi":', value: "CÔNG TY TNHH THƯƠNG MẠI DỊCH VỤ NHILE, đơn vị vận hành nền tảng giáo dục." },
                    { label: '"Dịch vụ":', value: "Các khóa học, chương trình đào tạo và dịch vụ hỗ trợ do N-Edu cung cấp." },
                    { label: '"Người dùng" hoặc "Bạn":', value: "Cá nhân hoặc tổ chức sử dụng dịch vụ của N-Edu." },
                    { label: '"Nội dung":', value: "Tất cả tài liệu, video, bài giảng, thông tin được cung cấp qua dịch vụ." }
                ]
            },
            registration: {
                title: "Đăng ký và tài khoản",
                items: [
                    { title: "Yêu cầu đăng ký:", list: ["Cung cấp thông tin chính xác, đầy đủ và cập nhật", "Đủ tuổi vị thành niên theo quy định pháp luật", "Có năng lực hành vi dân sự đầy đủ"] },
                    { title: "Trách nhiệm tài khoản:", list: ["Bảo mật thông tin đăng nhập", "Chịu trách nhiệm cho mọi hoạt động dưới tài khoản", "Thông báo ngay khi phát hiện truy cập trái phép"] }
                ]
            },
            courses: {
                title: "Đăng ký khóa học",
                items: [
                    { title: "Quy trình đăng ký:", list: ["Chọn khóa học phù hợp", "Điền form đăng ký với thông tin chính xác", "Hoàn tất thanh toán theo quy định", "Nhận xác nhận đăng ký thành công"] },
                    { title: "Điều kiện tham gia:", list: ["Hoàn đủ học phí trước khi khóa học bắt đầu", "Có đủ điều kiện về chuyên môn (nếu có)", "Cam kết tham gia đầy đủ các buổi học"] }
                ]
            },
            payment: {
                title: "Thanh toán",
                items: [
                    { title: "Phương thức thanh toán:", list: ["Chuyển khoản ngân hàng", "Thanh toán qua VNPay", "Thanh toán tại văn phòng", "Thanh toán quốc tế"] },
                    { title: "Chính sách hoàn tiền:", list: ["Hoàn 100% nếu hủy trước 7 ngày kể từ ngày đăng ký", "Hoàn 50% nếu hủy trong vòng 7 ngày trước khi khóa học bắt đầu", "Không hoàn tiền nếu hủy trong vòng 7 ngày trước khi khóa học bắt đầu"] }
                ]
            },
            rights: {
                title: "Quyền và nghĩa vụ của người dùng",
                user_rights: { title: "Quyền của bạn:", list: ["Truy cập nội dung khóa học đã đăng ký", "Nhận chứng nhận hoàn thành khóa học", "Học lại miễn phí khi có lớp mới", "Nhận hỗ trợ từ đội ngũ N-Edu", "Tham gia cộng đồng học viên"] },
                user_obligations: { title: "Nghĩa vụ của bạn:", list: ["Tuân thủ lịch học và nội quy", "Tôn trọng giảng viên và học viên khác", "Không chia sẻ tài liệu học tập trái phép", "Không sử dụng nội dung vi phạm bản quyền", "Thanh toán đầy đủ và đúng hạn"] }
            },
            ip: {
                title: "Sở hữu trí tuệ",
                subtitle: "Quy định về bản quyền:",
                list: ["Tất cả nội dung khóa học thuộc sở hữu của N-Edu và các đối tác", "Bạn được quyền sử dụng nội dung cho mục đích cá nhân học tập", "Nghiêm cấm sao chép, phân phối, hoặc thương mại hóa nội dung", "Vi phạm bản quyền có thể bị xử lý theo pháp luật", "N-Edu có quyền sử dụng hình ảnh học viên cho mục đích marketing"]
            },
            privacy: {
                title: "Bảo mật thông tin",
                content: "Chúng tôi cam kết bảo vệ thông tin cá nhân của bạn theo Chính sách bảo mật. Thông tin của bạn sẽ được:",
                list: ["Thu thập và sử dụng cho mục đích đã nêu rõ", "Bảo mật bằng các biện pháp kỹ thuật hiện đại", "Không chia sẻ với bên thứ ba mà không có sự đồng ý", "Lưu trữ trong thời gian cần thiết theo quy định"]
            },
            liability: {
                title: "Hạn chế trách nhiệm",
                no_liability: { title: "N-Edu không chịu trách nhiệm cho:", list: ["Kết quả học tập không đạt kỳ vọng", "Thiệt hại do lỗi kỹ thuật ngoài tầm kiểm soát", "Nội dung do bên thứ ba cung cấp", "Mất mát cơ hội kinh doanh"] },
                max_liability: { title: "Trách nhiệm tối đa:", content: "Trong mọi trường hợp, trách nhiệm của N-Edu không vượt quá số tiền bạn đã thanh toán cho dịch vụ." }
            },
            termination: {
                title: "Chấm dứt dịch vụ",
                edu_rights: { title: "Trường hợp N-Edu có quyền chấm dứt:", list: ["Vi phạm điều khoản sử dụng", "Cung cấp thông tin sai lệch", "Hành vi gian lận hoặc không trung thực", "Gây ảnh hưởng tiêu cực đến cộng đồng"] },
                user_rights: { title: "Quyền của người dùng:", content: "Bạn có quyền chấm dứt sử dụng dịch vụ bất cứ lúc nào, nhưng không được hoàn tiền theo chính sách đã nêu." }
            },
            dispute: {
                title: "Giải quyết tranh chấp",
                items: [
                    { title: "Thương lượng:", content: "Mọi tranh chấp sẽ được giải quyết trước tiên bằng thương lượng, hòa giải giữa các bên." },
                    { title: "Thẩm quyền:", content: "Trường hợp không thể thương lượng, tranh chấp sẽ được giải quyết tại Tòa án nhân dân có thẩm quyền tại Đà Nẵng." },
                    { title: "Luật áp dụng:", content: "Điều khoản này được điều chỉnh theo pháp luật nước Cộng hòa Xã hội Chủ nghĩa Việt Nam." }
                ]
            },
            general: {
                title: "Điều khoản chung",
                list: ["Điều khoản này thay thế mọi thỏa thuận trước đó giữa các bên", "Nếu bất kỳ điều khoản nào bị vô hiệu, các điều khoản còn lại vẫn có hiệu lực", "N-Edu có quyền thay đổi điều khoản và sẽ thông báo trước khi áp dụng", "Địa đổi có hiệu lực khi được đăng tải trên website", "Việc bạn tiếp tục sử dụng dịch vụ đồng nghĩa với việc chấp nhận thay đổi"]
            },
            contact: {
                title: "Thông tin liên hệ",
                items: [
                    { label: "Công ty:", value: "CÔNG TY TNHH THƯƠNG MẠI DỊCH VỤ NHILE" },
                    { label: "Mã số thuế:", value: "0317268736" },
                    { label: "Địa chỉ:", value: "Lô 3, khu B2-7, Khu đô thị phức hợp Halla Jade Residence, Phường Hải Châu, TP Đà Nẵng, Việt Nam" },
                    { label: "Email:", value: "nedu@nhi.sg" },
                    { label: "Điện thoại:", value: "(+84) 789785904" },
                    { label: "Website:", value: "https://nedu.nhi.sg" }
                ]
            },
            effectiveness: {
                title: "Hiệu lực",
                content: "Điều khoản sử dụng này có hiệu lực từ ngày 01/01/2024 và áp dụng cho tất cả người dùng sử dụng dịch vụ của N-Edu."
            }
        },
        back_home: "Quay lại trang chủ"
    },
    policy: {
        title: "Chính sách bảo mật",
        sections: {
            intro: {
                title: "Giới thiệu",
                content: "Tại N-Edu, chúng tôi cam kết bảo vệ thông tin cá nhân của bạn. Chính sách bảo mật này giải thích cách chúng tôi thu thập, sử dụng, và bảo vệ thông tin của bạn khi sử dụng dịch vụ của chúng tôi."
            },
            info_collected: {
                title: "Thông tin chúng tôi thu thập",
                personal: { title: "Thông tin cá nhân:", list: ["Họ và tên", "Email", "Số điện thoại", "Ngày sinh", "Giới tính", "Địa chỉ", "Username Telegram"] },
                study: { title: "Thông tin học tập:", list: ["Khóa học đã đăng ký", "Tiến độ học tập", "Kết quả bài kiểm tra", "Thời gian tham gia"] },
                tech: { title: "Thông tin kỹ thuật:", list: ["Địa chỉ IP", "Loại trình duyệt", "Thiết bị sử dụng", "Dữ liệu truy cập website"] }
            },
            purpose: {
                title: "Mục đích sử dụng thông tin",
                list: ["Cung cấp dịch vụ giáo dục và hỗ trợ học tập", "Gửi thông tin về khóa học và sự kiện", "Cải thiện chất lượng dịch vụ", "Xử lý thanh toán và xác nhận đăng ký", "Tuân thủ quy định pháp luật"]
            },
            security: {
                title: "Bảo mật thông tin",
                box_title: "Các biện pháp bảo mật chúng tôi áp dụng:",
                list: ["Mã hóa dữ liệu SSL/TLS cho tất cả kết nối", "Phân quyền truy cập thông tin", "Lưu trữ dữ liệu trên máy chủ bảo mật", "Backup dữ liệu định kỳ", "Quét bảo mật và cập nhật hệ thống thường xuyên", "Đào tạo nhân viên về bảo mật thông tin"]
            },
            sharing: {
                title: "Chia sẻ thông tin",
                content: "Chúng tôi cam kết không bán, thuê, hoặc cho thuê thông tin cá nhân của bạn cho bên thứ ba. Thông tin chỉ được chia sẻ trong các trường hợp sau:",
                list: ["Khi có sự đồng ý của bạn", "Để cung cấp dịch vụ (đối tác thanh toán, logistics)", "Khi yêu cầu của cơ quan pháp luật có thẩm quyền", "Để bảo vệ quyền lợi và an toàn của người dùng", "Trong trường hợp sáp nhập, mua bán doanh nghiệp"]
            },
            rights: {
                title: "Quyền của bạn",
                items: [
                    { title: "Quyền truy cập", content: "Bạn có quyền yêu cầu xem thông tin cá nhân mà chúng tôi lưu trữ về bạn." },
                    { title: "Quyền chỉnh sửa", content: "Bạn có quyền yêu cầu cập nhật hoặc sửa đổi thông tin cá nhân không chính xác." },
                    { title: "Quyền xóa", content: "Bạn có quyền yêu cầu xóa thông tin cá nhân trong các trường hợp được pháp luật cho phép." },
                    { title: "Quyền phản đối", content: "Bạn có quyền phản đối việc xử lý thông tin cá nhân của mình." }
                ]
            },
            cookies: {
                title: "Cookie",
                content1: "Website của chúng tôi sử dụng cookie để cải thiện trải nghiệm người dùng. Cookie giúp chúng tôi:",
                list: ["Nhớ thông tin đăng nhập", "Theo dõi tiến độ học tập", "Cá nhân hóa nội dung", "Phân tích lưu lượng truy cập"],
                content2: "Bạn có thể quản lý cookie trong cài đặt trình duyệt."
            },
            storage: {
                title: "Lưu trữ thông tin",
                content1: "Chúng tôi lưu trữ thông tin cá nhân của bạn trong thời gian cần thiết để:",
                list: ["Cung cấp dịch vụ giáo dục", "Thực hiện các nghĩa vụ pháp lý", "Giải quyết tranh chấp", "Bảo vệ quyền lợi của các bên"],
                content2: "Thời gian lưu trữ cụ thể tùy thuộc vào loại thông tin và mục đích sử dụng."
            },
            changes: {
                title: "Thay đổi chính sách",
                content: "Chúng tôi có thể cập nhật chính sách bảo mật này theo thời gian. Mọi thay đổi sẽ được thông báo trên website và qua email cho người dùng đã đăng ký."
            },
            contact: {
                title: "Liên hệ",
                content: "Nếu bạn có câu hỏi về chính sách bảo mật hoặc muốn thực hiện quyền của mình, vui lòng liên hệ:",
                items: [
                    { label: "Email:", value: "nedu@nhi.sg" },
                    { label: "Điện thoại:", value: "(+84) 789785904" },
                    { label: "Địa chỉ:", value: "Lô 3, khu B2-7, Khu đô thị phức hợp Halla Jade Residence, Phường Hải Châu, TP Đà Nẵng, Việt Nam" },
                    { label: "Mã số thuế:", value: "0317268736" }
                ]
            },
            effectiveness: {
                title: "Hiệu lực",
                content: "Chính sách bảo mật này có hiệu lực từ ngày 01/01/2024 và thay thế mọi chính sách bảo mật trước đó."
            }
        },
        back_home: "Quay lại trang chủ"
    },
    payment_guide: {
        title: "Hướng dẫn thanh toán",
        last_updated: "Cập nhật lần cuối ngày 18 tháng 11 năm 2024",
        steps: {
            step1: "Bước 1. Chọn khóa học và lựa chọn đăng ký",
            step2: "Bước 2. Điền thông tin và nhấn tiếp theo",
            step3: "Bước 3. Kiểm tra lại thông tin và nhấn thanh toán",
            step4: "Bước 4. Chọn phương thức thanh toán của VNPay và tiến hành thanh toán",
            step5: "Bước 5. Sau khi thanh toán thành công, bạn vào email để kiểm tra thông tin"
        }
    },
    cart: {
        empty_title: "Giỏ hàng của bạn đang trống",
        empty_desc: "Hãy thêm các khóa học bạn quan tâm vào giỏ hàng",
        continue_browsing: "Tiếp tục xem khóa học",
        title: "Giỏ hàng của bạn",
        summary_title: "Tóm tắt đơn hàng",
        total_items: "Tổng số khóa học:",
        total_price: "Tổng tiền:",
        checkout_btn: "Tiến hành thanh toán",
        continue_shopping: "Tiếp tục mua sắm",
        other_courses: "Các khóa học khác",
        view_all: "Xem tất cả khóa học",
        back_to_cart: "Quay lại giỏ hàng",
        view_cart: "Xem Giỏ Hàng",
        order_total_text: "Tổng giá trị đơn hàng của bạn là",
        checkout_button: "Thanh toán",
        delete_button: "Xóa"
    },
    checkout: {
        title: "Thanh toán",
        cart_items_title: "Các khóa học trong giỏ hàng",
        customer_info_title: "THÔNG TIN KHÁCH HÀNG",
        form: {
            name: "Họ và tên",
            email: "Email",
            phone: "Số điện thoại",
            telegram: "Username Telegram",
            birthdate: "Ngày sinh",
            gender: "Giới tính",
            address: "Địa chỉ",
            note: "Ghi chú",
            gender_options: {
                select: "Chọn giới tính",
                male: "Nam",
                female: "Nữ",
                other: "Khác"
            },
            placeholders: {
                name: "Nguyễn Văn A",
                email: "example@email.com",
                phone: "0912 345 678",
                telegram: "username",
                address: "Số nhà, tên đường, phường/xã, quận/huyện, tỉnh/thành phố",
                note: "Bạn có yêu cầu đặc biệt nào không?"
            }
        },
        continue_btn: "Tiếp tục",
        confirm_title: "XÁC NHẬN THÔNG TIN",
        selected_courses: "Khóa học đã chọn",
        personal_info: "Thông tin cá nhân",
        terms_agreement: "Bằng cách tích vào ô này, bạn xác nhận đã đọc, hiểu và đồng ý với",
        confirm_pay_btn: "Xác nhận và thanh toán",
        discount_code: "Mã giảm giá",
        apply_btn: "Áp dụng",
        applied_discount: "Đã áp dụng mã giảm giá:",
        subtotal: "Tạm tính:",
        discount: "Giảm giá:",
        total: "Tổng cộng:",
        processing: "Đang xử lý..."
    }
};
